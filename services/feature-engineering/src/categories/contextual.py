...
class ContextualFeatureComputer(BaseFeatureComputer):
    ...
    def compute(self, ctx: MatchContext, **kwargs) -> Dict[str, Optional[float]]:
        ...
        # Venue
        features["ctx__venue__is_neutral"] = 1.0 if ctx.metadata.get("neutral_venue") else 0.0
    ...
    def _compute_rest_days(self, features: Dict[str, Optional[float]], ctx: MatchContext) -> None:
    ...
    def _compute_referee(self, features: Dict[str, Optional[float]], ctx: MatchContext) -> None:
        if not ctx.referee:
            return

        try:
            result = self.db.execute_query(REFEREE_STATS, (ctx.referee,), fetch=True)
            if not result:
                return
            ...
            features["ctx__referee__is_strict"] = 1.0 if avg_yellows > 4 else 0.0
        except Exception as e:
            logger.error(f"Failed to compute referee features: {e}")

    def _compute_league_stats(self, features: Dict[str, Optional[float]], ctx: MatchContext) -> None:
        try:
            result = self.db.execute_query(
                LEAGUE_SEASON_STATS,
    ...
    def _compute_standings(self, features: Dict[str, Optional[float]], ctx: MatchContext) -> None:
        try:
        ...
        for side, pos, data in [
            ...
            features[f"ctx__{side}__is_bottom3"] = 1.0 if pos > total_teams - 3 else 0.0
        ...
        if home_data is not None and away_data is not None:
            features["ctx__points_diff"] = self._safe_get(home_data, "points") - self._safe_get(away_data, "points")

        if home_pos is not None and away_pos is not None:
            features["ctx__is_top_clash"] = 1.0 if home_pos <= 6 and away_pos <= 6 else 0.0
            features["ctx__is_relegation_battle"] = (
                1.0 if home_pos > total_teams - 5 and away_pos > total_teams - 5 else 0.0
            )
