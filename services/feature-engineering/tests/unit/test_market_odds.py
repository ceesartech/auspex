...
class TestMarketOddsComputer:
    ...
    def test_compute_with_odds(
        self, computer, match_context, mock_db,
        sample_odds, sample_opening_odds
    ):
        call_count = [0]
    ...
    def test_favourite_detection(
        self, computer, match_context, mock_db,
        sample_odds, sample_opening_odds
    ):
        mock_db.execute_query.side_effect = lambda q, p, fetch=False: (
            sample_odds if "DISTINCT ON" in q
            else sample_opening_odds if "is_opening" in q
            else sample_odds
        )
        features = computer.compute(match_context)
    ...
    def test_overround_non_negative(
        self, computer, match_context, mock_db,
        sample_odds, sample_opening_odds
    ):
        mock_db.execute_query.side_effect = lambda q, p, fetch=False: (
