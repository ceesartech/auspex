...
class FBrefScraper(BaseScraper):
    ...
    def _save_match_stats(self, match_data: Dict):
        ...
        league_id = self.db.get_or_create_league(
            name=match_data["league"], country="International", sport="soccer", season=match_data["season"]
        )

        # Get or create teams
        home_team_id = self.db.get_or_create_team(name=match_data["home_team"], league_id=league_id)

        away_team_id = self.db.get_or_create_team(name=match_data["away_team"], league_id=league_id)
        ...
        result = self.db.execute_query(query, (home_team_id, away_team_id, match_data["match_date"]), fetch=True)
        ...
        if result:
            ...
            update_query = """
                ...
                WHERE match_id = %s
            """
            self.db.execute_query(update_query, (match_data["home_score"], match_data["away_score"], match_id))
        else:
            # Create new match
            insert_query = """
                INSERT INTO matches
        ...
        if match_data.get("home_xg") is not None:
            self._save_xg_stats(match_id, home_team_id, away_team_id, match_data["home_xg"], match_data["away_xg"])

    def _save_xg_stats(self, match_id: int, home_team_id: int, away_team_id: int, home_xg: float, away_xg: float):
