...
with DAG(
    ...
    fetch_upcoming = BashOperator(
        ...
        bash_command=f"{DOCKER_EXEC} python /app/scripts/fetch_upcoming.py --days 7",
    )

    compute_features = BashOperator(
        task_id="compute_features",
        bash_command=f"{DOCKER_EXEC} python /app/scripts/compute_features.py --days 7",
    )

    precompute_predictions = BashOperator(
        task_id="precompute_predictions",
        bash_command=f"{DOCKER_EXEC} python /app/scripts/precompute_predictions.py --days 7",
    )
