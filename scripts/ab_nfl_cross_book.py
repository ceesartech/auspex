...
def _per_market_features(
    ...
    for match_id, g in wide.groupby("match_id"):
        ...
        consensus_line = (
            float(non_pinn["p_line"].mean()) if not non_pinn.empty else np.nan
        )
        consensus_impl = (
            float(non_pinn["p_impl"].mean()) if not non_pinn.empty else np.nan
        )
        ...
        rows.append({
            ...
            f"{prefix}_pinnacle_minus_consensus": (
                pinn_line - consensus_line
                if not (np.isnan(pinn_line) or np.isnan(consensus_line))
                else np.nan
            ),
            ...
            f"{prefix}_pinnacle_minus_consensus_implied_prob": (
                pinn_impl - consensus_impl
                if not (np.isnan(pinn_impl) or np.isnan(consensus_impl))
                else np.nan
            ),
        })
...
def compute_crossbook_features(snapshots: pd.DataFrame) -> pd.DataFrame:
    spread = _per_market_features(
        snapshots, "spread", "home", "away", "spread"
    )
    total = _per_market_features(
        snapshots, "total", "over", "under", "total"
    )
...
def run_market(
    ...
    crossbook_keys, database_url, crossbook_features, test_season,
):
    ...
    LOGGER.info(
        ...
        pinn_cov, len(merged), 100.0 * pinn_cov / max(1, len(merged)),
    )
...
def main():
    ...
    LOGGER.info(
        ...
        len(snapshots), snapshots["match_id"].nunique(),
    )
