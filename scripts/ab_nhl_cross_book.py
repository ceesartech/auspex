"""One-off A/B: do cross-book features help NHL across 4 markets?

NHL has the same multi-book structure as NFL and NBA (22-24 books
per match) across moneyline, puck_line (spread ±1.5), and total
5.5. Plus the niche NHL_REGULATION market (regulation-time winner
only, no OT/SO). This script tests all 4.

Same features as the NFL/NBA harnesses. Selection mapping:
  MONEYLINE  : home / away (devig pair)
  PUCK_LINE  : home / away (NHL spread, fixed at ±1.5)
  REGULATION : home / away (similar to moneyline but reg-only)
  TOTAL      : over / under (line 5.5)

Run on prod via:
    docker compose exec api python /app/scripts/ab_nhl_cross_book.py
"""
...
LOGGER = logging.getLogger("ab_nhl_cross_book")
        ...
        rows.append(
            {
                ...
                f"{prefix}_consensus_implied_prob": (float(impls.mean()) if len(impls) > 0 else np.nan),
            }
        )
    return pd.DataFrame(rows)
...
def run_market(
    ...
    # Skip markets whose target has more than 2 classes — the binary
    # XGB harness can't handle them. NHL_REGULATION is the
    # 3-way home/away/tie case; we'd need a multi-class brier helper
    # to test it. Skip for now.
    n_classes = train[target].nunique()
    if n_classes > 2:
        LOGGER.info(
            "Skipping %s — target has %d classes (binary harness only).",
            label,
            n_classes,
        )
        return
...
def main():
    ...
    LOGGER.info("Loading NHL snapshot streams...")
    ...
    try:
        ...
        engine.dispose()
    LOGGER.info("ML snapshots: %d / Spread+Total snapshots: %d.", len(ml_snapshots), len(st_snapshots))
    ...
    # NHL_NON_FEATURE_COLUMNS is the ML set; regulation has its own.
    run_market(
        "MONEYLINE",
        loader_name="load_nhl_moneyline_frame",
        non_feature_set_name="NHL_NON_FEATURE_COLUMNS",
        target_name="NHL_MONEYLINE_TARGET",
        market_keys=ML_KEYS,
    ...
    run_market(
        "REGULATION",
        loader_name="load_nhl_regulation_frame",
        non_feature_set_name="NHL_REGULATION_NON_FEATURE_COLUMNS",
        target_name="NHL_REGULATION_TARGET",
        market_keys=ML_KEYS,
        database_url=args.database_url,
        market_features=ml_features,
        test_season=args.test_season,
    ...
    run_market(
        "PUCK_LINE",
        loader_name="load_nhl_puck_line_frame",
        non_feature_set_name="NHL_PUCK_LINE_NON_FEATURE_COLUMNS",
        target_name="NHL_PUCK_LINE_TARGET",
    ...
    run_market(
        "TOTAL",
        loader_name="load_nhl_total_frame",
        non_feature_set_name="NHL_TOTAL_NON_FEATURE_COLUMNS",
        target_name="NHL_TOTAL_TARGET",
