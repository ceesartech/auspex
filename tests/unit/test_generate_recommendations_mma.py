"""Unit tests for generate_recommendations_mma — pure helpers.

Mirror of test_generate_recommendations_nfl. MMA has a single
...
class TestMarketDispatch:
    def test_only_moneyline_ensemble_in_v1(self):
        assert set(gr_mma.MMA_MARKETS.keys()) == {"ensemble_mma_ml"}

    def test_market_label_matches_taskspec(self):
        assert gr_mma.MMA_MARKETS["ensemble_mma_ml"][0] == "moneyline"
    ...
    def test_display_label_is_match_winner(self):
        ...
        assert gr_mma.MMA_MARKETS["ensemble_mma_ml"][1] == "Fight Winner"
    ...
    def test_labels_match_taskspec(self):
        ...
        assert gr_mma.MMA_LABELS["moneyline"] == ["home", "away"]


class TestRiskFactors:
    def test_longshot_flag_at_4x_or_more(self):
        # MMA +400 underdogs (~20% implied) are rare but happen
        # at the top of the draw — top-10 vs qualifier.
        assert "longshot" in gr_mma._risk_factors(prob=0.25, odds_decimal=4.0)
        assert "longshot" in gr_mma._risk_factors(prob=0.20, odds_decimal=5.0)
        assert "longshot" not in gr_mma._risk_factors(prob=0.55, odds_decimal=1.85)

    def test_low_model_prob_flag_under_45(self):
        assert "low_model_probability" in gr_mma._risk_factors(prob=0.40, odds_decimal=2.5)
        assert "low_model_probability" not in gr_mma._risk_factors(prob=0.50, odds_decimal=2.0)

    def test_clean_pick_has_no_risks(self):
        assert gr_mma._risk_factors(prob=0.55, odds_decimal=1.85) == []


class TestSharedMathReuse:
    def test_uses_soccer_kelly_and_ev(self):
        assert gr_mma.expected_value(0.55, 2.0) == 0.55 * 2.0 - 1.0
        assert gr_mma.kelly_fraction(0.55, 2.0) > 0
        assert gr_mma.kelly_fraction(0.30, 2.0) == 0.0


class TestKellyConstant:
    def test_quarter_kelly(self):
        assert gr_mma.KELLY_FRACTION == 0.25


class TestProbabilityCap:
    def test_cap_clips_overconfident_predictions(self):
        # MMA moneyline models often emit 0.90+ on heavy favorites
        # (Djokovic vs lower-ranked qualifier). Cap protects stake
        # sizing from over-confident tails.
        capped = gr_mma.cap_prob(0.92)
        assert capped == gr_mma.PROB_CAP_FOR_EV

    def test_cap_leaves_low_confidence_alone(self):
        assert gr_mma.cap_prob(0.65) == 0.65

    def test_cap_exactly_at_threshold_unchanged(self):
        assert gr_mma.cap_prob(0.80) == 0.80

    def test_cap_value_locked(self):
        # 0.80 leaves a safety margin against worst-bucket
        # overconfidence. Same value as NBA/NFL.
        assert 0.75 <= gr_mma.PROB_CAP_FOR_EV <= 0.85
    ...
    def test_capped_ev_smaller_than_raw_ev(self):
        ...
        raw_ev = gr_mma.expected_value(0.92, 1.50)
        capped = gr_mma.cap_prob(0.92)
        capped_ev = gr_mma.expected_value(capped, 1.50)
